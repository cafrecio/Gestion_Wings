...
**Severidad:** Alta
**Resolución:** El recibo de cuota es para el tutor del alumno y lo emite quien cobra la cuota — ADMIN u OPERATIVO. Por el modelo de permisos de Wings (`docs/02-contratos/PERMISOS-ROLES.md`), el operativo ve toda la cobranza de cuotas igual que en el historial (rubro Cuotas = `permitido_para OPERATIVO` = ambos), sin importar quién cobró. En `ReciboController::cuota()`: se bloquea solo al PROFESOR con `abort(403)`; ADMIN y OPERATIVO pasan. Probado con los 4 casos reales: admin 200, operativo que cobró 200, operativo que NO cobró 200, profesor 403.
**Corrección de criterio (2026-07-13):** una primera versión de este fix restringió al operativo a "sus propias cajas" — modelo mental equivocado (el operativo no ve "solo lo suyo"). Corregido el mismo día. Ver el contrato de permisos.
