...
| **FIN-08** | Reglas de revision de cobranza — decision parcial 17/09 | Media | **Carlos 17/09: la revision es 100% tarea del OPERATIVO**, hoy la ruta y el menu estan solo para ADMIN (`routes/web.php`, grupo `ensure.admin.web`). Falta decidir parciales, precio historico y que "Inactivo" no condone sin ADMIN (condonar es solo ADMIN). Observaciones previas se conservan |
