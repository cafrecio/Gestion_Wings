...
| **FIN-08** | Reglas de revision de cobranza — decision parcial 17/09, **"Inactivo" corregido 19/09** | Media | **Carlos 17/09: la revision es 100% tarea del OPERATIVO**, hoy la ruta y el menu estan solo para ADMIN (`routes/web.php`, grupo `ensure.admin.web`). **19/09: "Inactivo" ya no condona.** La cola pregunta una sola cosa —¿le generamos la deuda del mes?— y va hacia adelante: si se da de baja no se genera y el historial queda intacto; si continua, se genera. Condonar lo viejo es decision aparte del ADMIN. 5 pruebas en `RevisionCobranzaNoTocaElPasadoTest`; con el codigo anterior falla la que cuenta. Falta decidir parciales y precio historico. Planteo futuro: [suspender la cuota sin dar de baja](../05-pendientes/SUSPENSION-TEMPORAL-Y-VACACIONES.md) |
| **FIN-09** | Limites de fechas manuales — decidida 17/09, pendiente de implementar | Media | Carlos 17/09, precisado el 19/09: sin fechas futuras; **solo el mes en curso se carga sin aviso**. El registro del 17/09 tenia dos versiones del borde ("mes en curso y anterior normal" contra "cualquier mes anterior al actual"); Carlos zanjo el 19/09 a favor de la segunda: avisa desde el mes anterior inclusive. Mas viejo no se rechaza: **el movimiento lleva su fecha real** (ej. cobro por MP del 31/08 descubierto en septiembre) y **se liquida con la caja abierta de hoy**; las cajas ya controladas no se modifican. Se avisa en pantalla que es muy viejo y se manda mail y Telegram al ADMIN. Precision 17/09: el reporte no puede avisar de lo que todavia no se registro, asi que el aviso va **al cargar**, y el umbral es **cualquier mes anterior al actual**, porque ya ahi se cambia un resultado visto. Propuesta de Claude pendiente del OK de Carlos: guardar fecha del movimiento y fecha de carga, y que el reporte del mes viejo liste siempre los ajustes cargados despues, aunque la caja este validada. Consecuencia aceptada por Carlos: el reporte del mes de la fecha real cambia (agosto $20.000.000 pasa a $20.050.000). Aplica tambien al cobro de cuotas (`fecha_pago` hoy solo impide futuro). Hoy caja (`CajaWebController` ingreso/egreso manual) y Cashflow aceptan cualquier fecha. Laravel no tiene hoy envio de mail ni Telegram: solo los scripts del servidor |
