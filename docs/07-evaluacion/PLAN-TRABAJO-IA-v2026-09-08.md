...
| **FIN-09** | Limites de fechas manuales — decidida 17/09, pendiente de implementar | Media | Carlos 17/09: sin fechas futuras; mes en curso y anterior se cargan normal. Mas viejo no se rechaza: **el movimiento lleva su fecha real** (ej. cobro por MP del 31/08 descubierto en septiembre) y **se liquida con la caja abierta de hoy**; las cajas ya controladas no se modifican. Se avisa en pantalla que es muy viejo y se manda mail y Telegram al ADMIN. Consecuencia aceptada por Carlos: el reporte del mes de la fecha real cambia (agosto $20.000.000 pasa a $20.050.000). Aplica tambien al cobro de cuotas (`fecha_pago` hoy solo impide futuro). Hoy caja (`CajaWebController` ingreso/egreso manual) y Cashflow aceptan cualquier fecha. Laravel no tiene hoy envio de mail ni Telegram: solo los scripts del servidor |
