...
| **FIN-09** | Limites de fechas manuales — decidida 17/09, pendiente de implementar | Media | Carlos 17/09: sin fechas futuras; mes en curso y anterior se cargan normal. Mas viejo no se rechaza: se carga en la caja del dia con fecha de hoy, dejando la fecha real declarada en el detalle, se avisa en pantalla que es muy viejo y se manda mail y Telegram al ADMIN. Hoy caja (`CajaWebController` ingreso/egreso manual) y Cashflow aceptan cualquier fecha. Laravel no tiene hoy envio de mail ni Telegram: solo los scripts del servidor |
