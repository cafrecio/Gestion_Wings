...
### D4.0 — Columna muerta `pagos.forma_pago_id` (con su FK)
**Severidad:** Media
...
### D5.0 — Índice redundante en `deuda_cuotas`
**Severidad:** Media
...
### D6.0 — Estados de negocio como strings libres, sin dominio cerrado
**Severidad:** Media
**Dónde:** `deuda_cuotas.estado`, `cajas_operativas.estado`, `pagos.estado`, `alumnos_revision_cobranza.estado_revision`, `liquidaciones.estado`/`estado_pago` — todas columnas string con constantes definidas solo en los modelos.
**Qué pasa:** El conjunto de valores válidos vive únicamente en constantes PHP; la BD acepta cualquier string. Un typo en una constante, un seeder o una query manual puede meter un estado inválido que nadie detecta hasta que la lógica lo ignora silenciosamente. Ya hay indicios de esto (comentarios del tipo "DeudaCuota no tiene estado PARCIAL").
**Soluciones:**
